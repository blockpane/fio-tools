...
func (o *GetFeeVotesProducerUsdURL) Build() (*url.URL, error) {
	...
	var _path = "/fee/votes/{producer}/usd"

	producer := o.Producer
	if producer != "" {
		_path = strings.Replace(_path, "{producer}", producer, -1)
	} else {
		return nil, errors.New("producer is required on GetFeeVotesProducerUsdURL")
	}
...
func (o *GetFeeVotesProducerUsdURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetFeeVotesProducerUsdURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetFeeVotesProducerUsdURL")
	}
