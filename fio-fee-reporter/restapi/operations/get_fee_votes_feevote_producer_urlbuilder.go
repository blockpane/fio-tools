...
func (o *GetFeeVotesFeevoteProducerURL) Build() (*url.URL, error) {
	...
	var _path = "/fee/votes/feevote/{producer}"

	producer := o.Producer
	if producer != "" {
		_path = strings.Replace(_path, "{producer}", producer, -1)
	} else {
		return nil, errors.New("producer is required on GetFeeVotesFeevoteProducerURL")
	}
...
func (o *GetFeeVotesFeevoteProducerURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetFeeVotesFeevoteProducerURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetFeeVotesFeevoteProducerURL")
	}
